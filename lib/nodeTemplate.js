...
function getViteConfigTemplate(choice, config) {
  ...
  if ([1, 2].includes(choice.id)) dirname = path.resolve(__dirname, "./nodeTemplate2/vite.config.ejs");
...
async function getPackageTemplate(choice, config) {
  ...
  if ([1, 2].includes(choice.id)) _dependencies = dependenciesVite5;
  ...
  if ([1, 2].includes(choice.id)) _devDependencies = devDependenciesVite5;
  ...
  //     _devDependencies['sass'] = '1.77.6';
  //   } else if (key === '@vitejs/plugin-vue') {
  //     if (choice.id === 0) {
  //       _devDependencies['@vitejs/plugin-vue'] = '4.6.2';
  //     } else {
  //       _devDependencies['@vitejs/plugin-vue'] = '5.2.1';
  //     }
  //   } else {
  ...
  if ([1, 2].includes(choice.id)) dirname = path.resolve(__dirname, "./nodeTemplate2/package.ejs");
  const ejsTemplateData = fs.readFileSync(dirname);
